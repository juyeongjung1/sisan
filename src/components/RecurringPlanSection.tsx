        ...
        <div className="bg-slate-50 dark:bg-slate-800/40 p-4 rounded-xl border border-slate-200 dark:border-slate-700/80 space-y-3 animate-fade-in">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
              ...
              <span>自動積立の実行履歴 (直近)</span>
            </h3>
            <span className="text-[11px] text-slate-500">
              全 {accumulationLogs.length} 回の積立実行
            </span>
          </div>
              ...
              <div key={log.id} className="py-2 flex items-center justify-between">
                <div>
                  ...
                  <span className="text-[11px] text-slate-400">
                    {log.executedAt.replace('T', ' ').substring(0, 16)} • 自動反映済
                  </span>
                </div>
          ...
          return (
            ...
            >
              ...
              <div className="mt-3 pt-3 border-t border-slate-200 dark:border-slate-700/60 flex items-baseline justify-between">
                <div>
                  <span className="text-[10px] text-slate-400 uppercase font-semibold">毎月の積立額</span>
                ...
                <div className="text-right">
                  <span className="text-[10px] text-slate-400 uppercase font-semibold">積立日</span>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-white tracking-wide">
                将来資産シミュレーション (毎月 {formatCurrencyJpy(monthlyTotal)} 積立)
              </h3>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-1 bg-slate-800/90 p-1 rounded-xl border border-slate-700/80">
              <span className="text-[11px] text-slate-400 pl-1.5 pr-0.5 font-medium">期間:</span>
            ...
            <div className="flex items-center gap-1 bg-slate-800/90 p-1 rounded-xl border border-slate-700/80">
              <span className="text-[11px] text-slate-400 pl-1.5 pr-0.5 font-medium">想定年利:</span>
          ...
          {milestoneYears.map((yr) => {
            ...
            return (
              <div key={yr} className="bg-slate-800/60 p-2.5 rounded-lg border border-slate-700/50">
                <span className="text-slate-400 block text-[11px]">{yr}年後 想定総額</span>
                <span className={`font-bold text-sm ${yr === simulationHorizon ? 'text-emerald-400' : 'text-indigo-300'}`}>
                  {formatCurrencyJpy(pt?.total || 0)}
                </span>
                <span className="text-[10px] text-slate-500 block mt-0.5">
                  元本: {formatCurrencyJpy(pt?.invested || 0)}
                </span>
              </div>
            );
          })}
